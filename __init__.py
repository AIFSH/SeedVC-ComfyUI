...
ckpt_dir = os.path.join(aifsh_dir,"SeedVC")
...
bigvgan_44k_dir = os.path.join(ckpt_dir,"bigvgan_44k")
...
def download_models():
    ...
    if not os.path.exists(os.path.join(whisper_dir,"model.safetensors")):
        snapshot_download(repo_id="openai/whisper-small",
        local_dir=whisper_dir,
        allow_patterns=["*.safetensors","*json","*txt"],
        ignore_patterns=["*fp32*"])
    
    if not os.path.exists(os.path.join(bigvgan_44k_dir,"bigvgan_discriminator_optimizer.pt")):
        snapshot_download(repo_id="nvidia/bigvgan_v2_44khz_128band_512x",
                          local_dir=bigvgan_44k_dir,
                          allow_patterns=["*.json","*optimizer.pt","*generator.pt"])
...
def load_models(if_f0=False):
    ...
    if not if_f0:
        ...
        dit_checkpoint_path =os.path.join(seedvc_dir,"DiT_seed_v2_uvit_whisper_base_f0_44k_bigvgan_pruned_ft_ema.pth")
        dit_config_path =  os.path.join(seedvc_dir,"config_dit_mel_seed_uvit_whisper_base_f0_44k.yml")
        config = yaml.safe_load(open(dit_config_path, 'r'))
        model_params = recursive_munch(config['model_params'])
        ...
        bigvgan_44k_model = bigvgan.BigVGAN.from_pretrained(bigvgan_44k_dir, use_cuda_kernel=False)
    ...
    if not if_f0:
        ...
        return model_f0,sr,campplus_model,bigvgan_44k_model,whisper_model,whisper_feature_extractor,to_mel,rmvpe,hop_length
...
class SeedVC4SingNode:
    ...
    def gen_audio(self,source,target,pitch_shift,diffusion_steps,
                  length_adjust,inference_cfg_rate,auto_f0_adjust):
        
        model,sr,campplus_model,bigvgan_model,whisper_model,whisper_feature_extractor,to_mel,rmvpe,hop_length = load_models(if_f0=True)
        ...
        comfy_par = ProgressBar(cond.size(1))
        ...
        while processed_frames < cond.size(1):
            ...
            if processed_frames == 0:
                ...
                previous_chunk = vc_wave[0, -overlap_wave_len:]
                processed_frames += vc_target.size(2) - overlap_frame_len
            comfy_par.update(vc_target.size(2) - overlap_frame_len)
